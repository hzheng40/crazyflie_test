...
void genericCallback(double, double, double, double);
...
void genericCallback(/*float pressure,*/ double linear_x, double linear_y, double angular_x, double angular_y, int command) {
	count++;
	delay += 0.1;
...
void gestureCallback(const geometry_msgs::Quaternion &gesture_msg) {
	...
	if (gesture_hold >= 0.1) {
		...
		if (gesture_x < 0) {
			command = 4;
		} else if (gesture_x > 0.6) {
			command = 5;
		} else if (gesture_y > 0.5) {
			command = 2;
	...
	gestureUpdate = true;
	if (/*pressureUpdate && */imuUpdate && gestureUpdate) {
		genericCallback(/*pressure,*/ linear_x, linear_y, angular_x, angular_y, command);
	}
}
...
void imuCallback(const sensor_msgs::Imu &_msg) {
	...
	imuUpdate = true;
	if (imuUpdate && /*pressureUpdate &&*/ gestureUpdate) {
		genericCallback(/*pressure, */linear_x, linear_y, angular_x, angular_y, command);
	}
	//publish(0, 0, 0, 38000);
